...
void plansza::move_cards(std::vector<card> & baza, std::vector<card> & vec, const int & n)
{
    int counter = 0;
...
const bool plansza::isOneLower(const std::vector<card> & baza,
        int n) //Do sprawdzenia możliwośći przekładania wielu na planszę
{
    return baza[n].card_to_number() == (baza[n + 1].card_to_number() - 1);
}

const bool plansza::isOneHigher(const std::vector<card> & baza,
        const std::vector<card> & vec) // Do czynności przekładania na plansze
{
...
const int plansza::lengthOf(const std::vector<card> & baza)
{
    if(!baza.empty())
        return baza.size() - 1;
    return 0;
...
std::vector<card> & plansza::wybierz_pole(const int & n)
{
    switch(n)
    {
    case 1:return pole_1;
    case 2:return pole_2;
    case 3:return pole_3;
    case 4:return pole_4;
    case 5:return pole_5;
    case 6:return pole_6;
    case 7:return pole_7;
    }
}
