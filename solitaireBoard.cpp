...
void solitaireBoard::move(talia & t, plansza & p,const int & n) //dla talii
{
    t.moveCard(p.wybierz_pole(n));
}

void solitaireBoard::move(talia & t, segregacja & s,const int & n) //dla talii
{
    t.moveCard(s.wybierz_pole(n));
}

void solitaireBoard::move(plansza & baza, plansza & cel,const int & bn,const int & cn,const int & ilosc_kart) //dla planszy
{
    baza.move_cards(baza.wybierz_pole(bn), cel.wybierz_pole(cn), ilosc_kart);
}

void solitaireBoard::move(plansza & p, segregacja & s,const int & pn,const int & sn)//dla planszy
{
    p.put_card(p.wybierz_pole(pn),s.wybierz_pole(sn));
}

void solitaireBoard::move(segregacja & s, plansza & p,const int & sn,const int & pn)//dla segregacji
{
    s.put_card(s.wybierz_pole(sn),p.wybierz_pole(pn));
}
const int solitaireBoard::getStackSize(const talia & t)
{
